...
double a [accounts][2];     //The ammount of accounts that is possible
...
void withdrawl(int id, double amount);			//withdrawl money from the account if the account exist and it has enough money
void close(int id);								//close one account
void interest(int ribit);							//insert the money from the interest to all open bank accounts
